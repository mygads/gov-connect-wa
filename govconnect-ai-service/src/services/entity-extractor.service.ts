...
/**
 * Extract citizen data from conversation history via Channel Service API
 * 
 * CONSOLIDATED from:
 * - unified-message-processor.service.ts::extractCitizenDataFromHistory
 * - two-layer-orchestrator.service.ts::extractCitizenDataFromHistoryInternal
 * 
 * @param userId - User ID (wa_user_id or session_id)
 * @param options - Optional configuration
 * @returns Extracted citizen data or null
 */
export async function extractCitizenDataFromHistory(
  userId: string,
  options: { limit?: number; timeout?: number } = {}
): Promise<CitizenData | null> {
  const { limit = 50, timeout = 5000 } = options;
  ...
  try {
    ...
    const response = await axios.get(url, {
      params: { wa_user_id: userId, limit },
    ...
    logger.debug('[EntityExtractor] Extracting from history', { 
      userId, 
      messageCount: userMessagesList.length,
      textLength: userMessages.length,
    });
    ...
    // If no name found with patterns, check for standalone name
    if (!result.nama_lengkap) {
      const excludeWords = [
        'ya', 'iya', 'ok', 'oke', 'tidak', 'bukan', 'mau', 'ingin', 'sudah', 'belum',
        'sip', 'siap', 'baik', 'terima', 'kasih', 'halo', 'hai', 'hi', 'hello',
        'selamat', 'pagi', 'siang', 'sore', 'malam', 'assalamualaikum', 'permisi',
        'maaf', 'tolong', 'bantu', 'bisa', 'gimana', 'bagaimana', 'apa', 'kapan',
        'dimana', 'siapa', 'kenapa', 'mengapa',
      ];
      
      for (const msg of userMessagesList) {
        // Check if message is a potential name (1-4 words, all letters, 2-50 chars)
        if (/^[A-Za-z]+(?:\s+[A-Za-z]+){0,3}$/.test(msg) && msg.length >= 2 && msg.length <= 50) {
    ...
    const addressPatterns = [
      /(?:alamat|tinggal\s+di|domisili|lokasi)[\s:]+(.+?)(?:\s*[,.]?\s*(?:untuk|mau|nik|hp|nama|no)|$)/i,
      /tinggal\s+di\s+(.+?)(?:\s*,?\s*(?:untuk|mau|nik|hp)|\s*$)/i,
    ];
    ...
    for (const pattern of addressPatterns) {
      ...
      if (match && match[1] && match[1].length >= 5) {
        ...
        // Remove complaint keywords if accidentally captured
    ...
    logger.debug('[EntityExtractor] Extraction result', { userId, result });
    
    return Object.keys(result).length > 0 ? result : null;
  } catch (error: any) {
    logger.warn('[EntityExtractor] Failed to extract from history', { userId, error: error.message });
    return null;
  }
}

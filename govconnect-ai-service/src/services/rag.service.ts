...
// =============================================================================
// UTILITY FUNCTIONS (for future use or multi-hop queries)
// =============================================================================

/**
 * Format RAG context for insertion into system prompt
 * Adjusts instructions based on confidence level
 * 
 * NOTE: Currently not used - context is built directly in context-builder.service.ts
 * Keeping for potential future use with custom prompt building
 * 
 * @param context - RAG context object
 * @returns Formatted string for prompt injection
 */
export function formatContextForPrompt(context: RAGContext): string {
  if (context.totalResults === 0 || !context.contextString) {
    return '';
  }

  // Adjust instructions based on confidence
  const confidenceLevel = context.confidence?.level || 'medium';
  let instructions: string;

  if (confidenceLevel === 'high') {
    instructions = `INSTRUCTIONS:
- Answer based PRIMARILY on the knowledge above
- The information is highly relevant to the question
- Don't add information not present in the knowledge`;
  } else if (confidenceLevel === 'medium') {
    instructions = `INSTRUCTIONS:
- Use the knowledge above as your primary source
- You may supplement with general knowledge if needed
- Clearly indicate if you're using general knowledge`;
  } else {
    instructions = `INSTRUCTIONS:
- The knowledge above may be partially relevant
- Use your general knowledge to provide a complete answer
- Prioritize information from the knowledge when applicable`;
  }

  return `
KNOWLEDGE BASE CONTEXT (Confidence: ${confidenceLevel}):
...
/**
 * Combine multiple RAG contexts (for multi-hop queries)
 * 
 * NOTE: Currently not used - keeping for future multi-step RAG implementation
 * Use case: Complex questions requiring multiple searches
 * 
 * @param contexts - Array of RAG contexts
 * @returns Combined context
 */
export function combineContexts(...contexts: RAGContext[]): RAGContext {
  const allChunks: VectorSearchResult[] = [];
  let totalSearchTime = 0;

  for (const ctx of contexts) {
    allChunks.push(...ctx.relevantChunks);
    totalSearchTime += ctx.searchTimeMs;
  }

  // Deduplicate and re-sort
  const uniqueChunks = deduplicateResults(allChunks);
  const sortedChunks = uniqueChunks.sort((a, b) => b.score - a.score);

  return {
    relevantChunks: sortedChunks,
    contextString: buildContextString(sortedChunks),
...
function deduplicateResults(results: VectorSearchResult[]): VectorSearchResult[] {
  ...
  for (const result of results) {
    ...
    const hash = result.content.substring(0, 100).toLowerCase().replace(/\s+/g, ' ');
